import { Alert, Button, Card, Col, Collapse, Row, Table, message } from "antd";
...
export default function DetailIdentMhs() {
  ...
  const getHistories = async () => {
    setIsLoading(true);
    try {
      const resHistory = await axios.get(`/histories?userId=${currentUser?._id}`);

      setDetail(resHistory.data[0]);
    } catch (error: any) {
  ...
  return (
    <AppLayout title={`Hasil`}>
      {isLoading && <LoadingComp />}
      {!isLoading && detail?.payload && (
        <>
      ...
      {!isLoading && !detail?.payload && (
        <Alert message={"Data masih kosong"} type="info" showIcon style={{ width: "100%" }} />
      )}
    </AppLayout>
  );
}
